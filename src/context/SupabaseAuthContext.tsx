...
export function SupabaseAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      try {
        ...
        if (data.session?.user) {
          ...
          const metaRole = (data.session.user.user_metadata as any)?.role as 'creator' | 'brand' | undefined;
    ...
    const { data } = supabase.auth.onAuthStateChange((event, session) => {
      if (session?.user) {
        ...
        const metaRole = (session.user.user_metadata as any)?.role as 'creator' | 'brand' | undefined;
        const savedRole = (metaRole as 'creator' | 'brand') || (localStorage.getItem('userRole') as 'creator' | 'brand' | null);
        setRoleState(savedRole);
      } else {
        setUser(null);
  ...
  const signIn = async (email: string, password: string) => {
    ...
    try {
      const pending = localStorage.getItem('userRolePending') as 'creator' | 'brand' | null;
      if (pending && data.user) {
        const { error: updErr } = await supabase.auth.updateUser({ data: { role: pending } });
        ...
        const metaRole = (data.user?.user_metadata as any)?.role as 'creator' | 'brand' | undefined;
        if (metaRole) {
