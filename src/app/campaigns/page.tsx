...
export default function CampaignsPage() {
    ...
    /**
     * Fetch campaigns:
     * - BRAND: Direct Firestore read (own campaigns) — allowed by security rules
     * - CREATOR: Cloud Function call — returns server-scored matches with matchScore
     */
    const fetchCampaigns = useCallback(async (loadMore = false) => {
        ...
        try {
            if (role === "creator") {
                // Creator uses Cloud Function — scores computed server-side
                // using minRatePrivate which is NEVER exposed to frontend
                ...
                setHasMore(false); // CF returns all at once
            } else {
                // Brand reads their own campaigns directly (read-only, allowed by rules)
                let campaignsQuery;
                if (loadMore && lastDoc) {
                ...
                const newCampaigns = snap.docs.map((doc) => ({
                    id: doc.id,
                    ...doc.data(),
                    createdAt:
                                    ...
                                    <Card className="border-border/50 hover:border-primary/30 transition-all duration-300 h-full flex flex-col group cursor-pointer">
                                        <CardContent className="p-6 flex-1 flex flex-col">
                                            ...
                                            {campaign.matchScore != null && (
                                                <div
                                                    className={`mt-3 flex items-center gap-1.5 text-xs font-medium ${getMatchColor(campaign.matchScore)}`}
                                                >
