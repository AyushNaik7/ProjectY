...
exports.getCreatorsForCampaign = exports.onCampaignCreated = exports.updateRequestStatus = exports.createRequest = exports.getMatchedCampaigns = exports.createCampaign = exports.completeBrandOnboarding = exports.completeCreatorOnboarding = exports.initializeUser = void 0;
...
exports.initializeUser = functions.https.onCall(async (data, context) => {
    ...
    if (!role || !["creator", "brand"].includes(role)) {
        throw new functions.https.HttpsError("invalid-argument", "Role must be 'creator' or 'brand'");
    }
    ...
    if (userDoc.exists) {
        ...
        if ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.onboardingComplete) {
            throw new functions.https.HttpsError("failed-precondition", "Cannot change role after onboarding is complete");
        }
 ...
 */
exports.completeCreatorOnboarding = functions.https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be logged in");
    }
    ...
    const userDoc = await firestore.doc(`users/${uid}`).get();
    if (!userDoc.exists || ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== "creator") {
        throw new functions.https.HttpsError("permission-denied", "Only creators can complete creator onboarding");
    }
    if ((_b = userDoc.data()) === null || _b === void 0 ? void 0 : _b.onboardingComplete) {
        throw new functions.https.HttpsError("failed-precondition", "Onboarding already completed");
    }
    // ---- Server-side input validation ----
    const { name, instagramHandle, niche, followers, avgViews, engagementRate, minRatePrivate, } = data;
    ...
    if (!instagramHandle || typeof instagramHandle !== "string") {
        throw new functions.https.HttpsError("invalid-argument", "Instagram handle is required");
    }
    if (!niche || typeof niche !== "string") {
        throw new functions.https.HttpsError("invalid-argument", "Niche is required");
    }
    if (typeof followers !== "number" || followers < 0) {
        throw new functions.https.HttpsError("invalid-argument", "Followers must be a non-negative number");
    }
    if (typeof avgViews !== "number" || avgViews < 0) {
        throw new functions.https.HttpsError("invalid-argument", "Average views must be a non-negative number");
    }
    if (typeof engagementRate !== "number" ||
        engagementRate < 0 ||
        engagementRate > 100) {
        throw new functions.https.HttpsError("invalid-argument", "Engagement rate must be between 0 and 100");
    }
    if (typeof minRatePrivate !== "number" || minRatePrivate < 0) {
        throw new functions.https.HttpsError("invalid-argument", "Minimum rate must be a non-negative number");
    }
 ...
 */
exports.completeBrandOnboarding = functions.https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be logged in");
    }
    const uid = context.auth.uid;
    const userDoc = await firestore.doc(`users/${uid}`).get();
    if (!userDoc.exists || ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== "brand") {
        throw new functions.https.HttpsError("permission-denied", "Only brands can complete brand onboarding");
    }
    if ((_b = userDoc.data()) === null || _b === void 0 ? void 0 : _b.onboardingComplete) {
        throw new functions.https.HttpsError("failed-precondition", "Onboarding already completed");
    }
    const { brandName, category, budgetMin, budgetMax, website, description } = data;
    if (!brandName ||
        typeof brandName !== "string" ||
        brandName.trim().length < 2) {
        throw new functions.https.HttpsError("invalid-argument", "Brand name must be at least 2 characters");
    }
    if (!category || typeof category !== "string") {
        throw new functions.https.HttpsError("invalid-argument", "Category is required");
    }
    if (typeof budgetMin !== "number" || budgetMin < 0) {
        throw new functions.https.HttpsError("invalid-argument", "Minimum budget must be a non-negative number");
    }
    if (typeof budgetMax !== "number" || budgetMax < 0) {
        throw new functions.https.HttpsError("invalid-argument", "Maximum budget must be a non-negative number");
    }
    if (budgetMax < budgetMin) {
        throw new functions.https.HttpsError("invalid-argument", "Maximum budget must be >= minimum budget");
    }
    const batch = firestore.batch();
...
exports.createCampaign = functions.https.onCall(async (data, context) => {
    ...
    const userDoc = await firestore.doc(`users/${uid}`).get();
    if (!userDoc.exists || ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== "brand") {
        throw new functions.https.HttpsError("permission-denied", "Only brands can create campaigns");
    }
    if (!((_b = userDoc.data()) === null || _b === void 0 ? void 0 : _b.onboardingComplete)) {
        throw new functions.https.HttpsError("failed-precondition", "Complete brand onboarding first");
    }
    ...
    if (!["Reel", "Post", "Story"].includes(deliverableType)) {
        throw new functions.https.HttpsError("invalid-argument", "Deliverable must be Reel, Post, or Story");
    }
    if (typeof budget !== "number" || budget <= 0) {
        throw new functions.https.HttpsError("invalid-argument", "Budget must be a positive number");
    }
    if (!timeline || typeof timeline !== "string") {
        throw new functions.https.HttpsError("invalid-argument", "Timeline is required");
    }
    if (!niche || typeof niche !== "string") {
        throw new functions.https.HttpsError("invalid-argument", "Niche is required");
    }
 ...
 */
exports.getMatchedCampaigns = functions.https.onCall(async (_data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be logged in");
    }
    ...
    const userDoc = await firestore.doc(`users/${uid}`).get();
    if (!userDoc.exists || ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== "creator") {
        throw new functions.https.HttpsError("permission-denied", "Only creators can view matches");
    }
    ...
    const matchedCampaigns = campaignsSnap.docs.map((doc) => {
        ...
        const match = (0, matching_1.computeMatchScore)(creator, campaign, doc.id);
...
exports.createRequest = functions.https.onCall(async (data, context) => {
    ...
    const userDoc = await firestore.doc(`users/${uid}`).get();
    if (!userDoc.exists || ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== "brand") {
        throw new functions.https.HttpsError("permission-denied", "Only brands can create requests");
    }
    const { campaignId, creatorId } = data;
    if (!campaignId || !creatorId) {
        throw new functions.https.HttpsError("invalid-argument", "campaignId and creatorId are required");
    }
    // Server-side validation (campaign exists, active, belongs to brand, no duplicates)
    const validation = await (0, requests_1.validateNewRequest)({ campaignId, creatorId, brandId: uid }, firestore);
    if (!validation.valid) {
        throw new functions.https.HttpsError("failed-precondition", validation.error || "Validation failed");
    }
    ...
    if (creatorDoc.exists && campaignDoc.exists) {
        const match = (0, matching_1.computeMatchScore)(creatorDoc.data(), campaignDoc.data(), campaignId);
        matchScore = match.score;
    }
...
exports.updateRequestStatus = functions.https.onCall(async (data, context) => {
    ...
    const userDoc = await firestore.doc(`users/${uid}`).get();
    if (!userDoc.exists || ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== "creator") {
        throw new functions.https.HttpsError("permission-denied", "Only creators can respond to requests");
    }
    const { requestId, status } = data;
    if (!requestId || !["accepted", "rejected"].includes(status)) {
        throw new functions.https.HttpsError("invalid-argument", "requestId and status (accepted/rejected) are required");
    }
    const result = await (0, requests_1.processRequestUpdate)(requestId, status, uid, firestore);
    if (!result.success) {
        throw new functions.https.HttpsError("failed-precondition", result.error || "Update failed");
    }
    return { success: true };
});
// ============================================
...
exports.onCampaignCreated = functions.firestore
    ...
    try {
        const matches = await (0, matching_1.computeMatchesForCampaign)(campaignId, campaign, firestore);
        // Store top matches as sub-collection for quick retrieval
        const batch = firestore.batch();
        const matchesRef = firestore.collection(`campaigns/${campaignId}/matches`);
        ...
        await batch.commit();
        console.log(`Computed ${matches.length} matches for campaign ${campaignId}`);
    }
    catch (error) {
 ...
 */
exports.getCreatorsForCampaign = functions.https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be logged in");
    }
    ...
    const userDoc = await firestore.doc(`users/${uid}`).get();
    if (!userDoc.exists || ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== "brand") {
        throw new functions.https.HttpsError("permission-denied", "Only brands can view creators");
    }
    ...
    const campaignDoc = await firestore.doc(`campaigns/${campaignId}`).get();
    if (!campaignDoc.exists || ((_b = campaignDoc.data()) === null || _b === void 0 ? void 0 : _b.brandId) !== uid) {
        throw new functions.https.HttpsError("permission-denied", "Campaign not found or unauthorized");
    }
    ...
    const matchesSnap = await firestore
        ...
        .limit(20)
        .get();
    const creators = await Promise.all(matchesSnap.docs.map(async (matchDoc) => {
