-- ============================================================
-- FIXED MIGRATION - Run this instead of RUN_ALL_MIGRATIONS.sql
-- This version is compatible with your existing schema
-- ============================================================
...
-- ============================================================
-- Migration 014: Conversations & Messages (FIXED)
-- ============================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'conversations_brand_creator_campaign_key'
  ) THEN
    ALTER TABLE conversations 
    ADD CONSTRAINT conversations_brand_creator_campaign_key 
...
-- 6. RLS Policies for conversations (simplified for compatibility)
...
-- 7. RLS Policies for messages (simplified for compatibility)
...
-- ============================================================
-- Migration 015: Notifications (FIXED)
-- ============================================================
...
-- ============================================================
-- Migration 016: Creator Portfolio (FIXED)
-- ============================================================
...
-- ============================================================
-- Migration 017: Reviews & Ratings (FIXED)
-- ============================================================
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'reviews_collaboration_reviewer_key'
  ) THEN
    ALTER TABLE reviews 
    ADD CONSTRAINT reviews_collaboration_reviewer_key 
...
-- ============================================================
-- Migration 018: Escrow Payments (FIXED)
-- ============================================================
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'payments_collaboration_request_id_key'
  ) THEN
    ALTER TABLE payments 
    ADD CONSTRAINT payments_collaboration_request_id_key 
...
BEGIN
  ...
  IF v_tables = 7 AND v_realtime = 3 THEN
  ...
  ELSE
    RAISE NOTICE '⚠️  Some items may need attention';
  END IF;
END $$;
